...
void sbi_terminate_ipi_local(struct sbi_tlb_info *__info) {
  struct sbi_terminate_ipi_info* info = (struct sbi_terminate_ipi_info *) __info;
  if (cpu_is_enclave_context()) {
    cpu_set_to_terminate(1);
    try_terminate_enclave((uintptr_t *)info->regs);
  }
}
...
void send_and_sync_terminate_ipi(uintptr_t encl_mask, uintptr_t *regs) // TODO add mask?
{
  ...
  SBI_TLB_INFO_INIT(&tlb_info, (unsigned long)regs, 0, 0, 0,
